...
const ConstraintForm = () => {
  ...
  const fetchTemplates = async () => {
    try {
      const response = await axios.get('http://54.162.87.128/proxy/apis/templates.gatekeeper.sh/v1beta1/constrainttemplates', {
        headers: {
  ...
  const handleSubmit = (values) => {
    const url = `http://54.162.87.128/proxy/apis/constraints.gatekeeper.sh/v1beta1/`+template;
