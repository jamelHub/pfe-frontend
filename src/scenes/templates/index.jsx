...
const Templates = () => {
  ...
  useEffect(() => {
    const baseUrl = `http://54.162.87.128/proxy/apis/templates.gatekeeper.sh/v1beta1/constrainttemplates`;
    ...
    <Box>
      ...
      {list.length === 0 ? (
        ...
        <div className="flex flex-col gap-2">
          {list.map((n) => (
            <Card key={n} nameT={n} deleteConstraintTemplate={() => deleteConstraintTemplate(n)}></Card>
          ))}
        </div>
      )}
    </Box>
